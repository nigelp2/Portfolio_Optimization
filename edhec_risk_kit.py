...
def get_ind_returns():
    """
    Load and format ken french 30 Industry portoflios value weighted monthly returns
    """
...
def semideviation(r):
    """
    returns the semideviation aka negative semideviation of r must be a Series or DataFrame
    """
    is_negative = r<0
...
def is_normal(r, level=0.01):
    ...
    returns True if the hypothesis of normaility is acpeted, False otherwise
    """
...
def plot_ef2(n_points, er, cov):
    ...
    if er.shape[0] !=2 or er.shape[0] != 2:
        raise ValueError("Plot_ef2 can only plot 2 asset")
...
def msr(riskfree_rate, er, cov):
   ...
   RiskFree rate +ER + COV -> W
    ...
    def neg_sharpe_ratio(weights, riskfree_rate, er, cov):
        """
        returns the weights of the portfolio that gives you the maximum sharpe ratio given the riskfree rate
        and expected returns and a covar matrix
        """
...
def gmv(cov):
    """
    Returns the weights of the global min vol portfolio 
    given the covar matrix
    """
    n = cov.shape[0]
...
def plot_ef(n_points, er, cov, show_cml = False, style = '.-', riskfree_rate=0, show_ew= False, show_gmv = False):
    ...
    vols = [portfolio_vol(w, cov) for w in weights]
    ef = pd.DataFrame({'Returns': rets, 'Vol': vols})
    ax = ef.plot.line(x = 'Vol', y = 'Returns', style = '.-')
    if show_ew:
        n = er.shape[0]
